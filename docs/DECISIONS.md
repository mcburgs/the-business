...
**Status:** Accepted and verified in Phase H
**Decision:** The project uses `canvas_items` + `expand`, a 720x720 design base and handheld `sensor` orientation. Pixel 9a / Android 17 acceptance confirmed both portrait and landscape are functionally viable, including repeated rotation, so Phase H retains responsive dual-orientation support rather than imposing a lock. Presentation may switch layout by available viewport geometry; orientation has no domain meaning.
**Reason:** The governing phase required portrait-vs-landscape judgment from the actual Pixel rather than aesthetic decree. Physical evidence confirmed both postures work. Landscape remains visually dense, which is recorded as Phase-I presentation debt rather than a Phase-H architectural blocker.
