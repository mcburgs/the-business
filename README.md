...
This repository is currently in **Phase H: Pixel / Android Target Build and Integration**. The accepted Phase G/G→H map-first client is being adapted for real Android lifecycle, persistence, touch and phone presentation without changing simulation authority. The off-device Phase-H candidate is green; matching Android export templates/SDK and physical Pixel evidence remain required before Phase H can be marked verified.
...
7. Do not certify Phase H from an APK alone. Complete the physical Pixel checklist in `docs/PHASE_H_DEVICE_RESULT.md`.
