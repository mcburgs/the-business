...
func _ready() -> void:
    _diagnostics = DiagnosticLog.new(); var probe: Variant = BootProbe.new(); _diagnostics.info("bootstrap", "APP_SHELL_READY", "Application shell initialized.", probe.collect())
    _session = CampaignSession.new(); var started: Dictionary = _session.call("start", DEFAULT_CAMPAIGN_DIRECTORY, 424242)
    if not bool(started.get("passed", false)):
        push_error("Phase G campaign session failed to initialize: " + str(started.get("errors", []))); _show_startup_failure(started.get("errors", [])); return
    _home.call("bind_session", _session); call_deferred("_handle_capture_request")
...
func _handle_capture_request() -> void:
    ...
    if not path.is_empty():
        var error := image.save_png(path); if error != OK: push_error("Phase G screenshot capture failed: " + path)
    get_tree().quit()
